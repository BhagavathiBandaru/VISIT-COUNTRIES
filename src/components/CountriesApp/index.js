...
import VisitedCountriesBlock from '../VisitedCountriesBlock'
...
class CountriesApp extends Component {
  ...
  onClickRemoveDataBtn = id => {
    const {countriesList} = this.state
    const deleteCountry = countriesList.map(eachCountry => {
      if (eachCountry.id === id) {
        const removeCountry = {
          ...eachCountry,
          isVisited: !eachCountry.isVisited,
    ...
    this.setState({countriesList: deleteCountry})
  }
      ...
      <div className="app-container">
        ...
        <ul className="list-container">
          {countriesList.map(eachCountry => (
            <CountriesListBlock
              key={eachCountry.id}
              countryData={eachCountry}
              onClickToVisitCountryBtn={this.onClickToVisitCountryBtn}
            />
          ))}
        </ul>
        <h1 className="visit-heading">Visited Countries</h1>
        <ul className="visited-countries-list">
          {updatedList.length === 0 ? (
            <p className="no-country-visit">No Countries Visited Yet!</p>
          ) : (
            updatedList.map(eachItem => (
              <VisitedCountriesBlock
                key={eachItem.id}
                visitedCountryData={eachItem}
                onClickRemoveDataBtn={this.onClickRemoveDataBtn}
              />
            ))
          )}
        </ul>
